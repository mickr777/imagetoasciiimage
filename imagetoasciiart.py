...
class ImageToDetailedASCIIArtInvocation(BaseInvocation):
    input_image: ImageField = InputField(description="Input image to convert to detailed ASCII art")
    font_spacing: int = InputField(default=6, description="Font size for the ASCII art characters")
    detailed_mode: bool = InputField(default=True, description="Enable detailed mode (default: true)")
        ...
        for y in range(num_rows):
            for x in range(num_cols):
                ...
                if color_mode:
                    ...
                    draw.text((x * font_spacing, y * font_spacing), ascii_char, fill=int(pixel_value))

        return ascii_art_image

    def invoke(self, context: InvocationContext) -> ImageOutput:
        input_image = context.services.images.get_pil_image(self.input_image.image_name)
        detailed_ascii_art_image = self.image_to_detailed_ascii_art(input_image, self.font_spacing, self.detailed_mode, self.color_mode)
